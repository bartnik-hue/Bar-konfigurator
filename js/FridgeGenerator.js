...
export class FridgeGenerator {
    ...
    /**
     * Generator fotorealistycznego modelu jednodrzwiowej lodówki barowej (połowa szerokości - 0.5m).
     */
    static createSingleFridgeModel() {
        const fridgeGroup = new THREE.Group();
        fridgeGroup.name = 'SingleFridgeRoot';

        // Wymiary jednodrzwiowej lodówki (Slim): 0.50m x 2.15m x 0.65m
        const width = 0.50;
        const height = 2.15;
        const depth = 0.65;
        const wallThickness = 0.04;
        ...
        for (let i = 1; i <= shelfCount; i++) {
            ...
            const bottleSpacing = (width - 0.16) / bottlesPerRow;
            for (let b = 0; b < bottlesPerRow; b++) {
                const bX = -width / 2 + 0.08 + b * bottleSpacing + bottleSpacing / 2;
                const bHeight = 0.18;
                const bottleGeom = new THREE.CylinderGeometry(0.028, 0.03, bHeight, 12);
                const bottleMat = new THREE.MeshStandardMaterial({
                    color: bottleColors[(i + b) % bottleColors.length],
        ...
        // 3. Pojedyncze przeszklone drzwi jednoskrzydłowe
        ...
        // Szyba
        const paneGeom = new THREE.BoxGeometry(doorWidth - frameThickness * 2, doorHeight - frameThickness * 2, 0.012);
        const glassPane = new THREE.Mesh(paneGeom, glassMat);
        glassPane.position.set(0, doorY, doorZ);
        fridgeGroup.add(glassPane);

        // Obramowanie szyby: poziome (góra i dół)
        const hFrameGeom = new THREE.BoxGeometry(doorWidth, frameThickness, 0.025);
        const topF = new THREE.Mesh(hFrameGeom, blackMetalMat);
        topF.position.set(0, doorY + doorHeight / 2 - frameThickness / 2, doorZ);
        fridgeGroup.add(topF);

        const botF = new THREE.Mesh(hFrameGeom, blackMetalMat);
        botF.position.set(0, doorY - doorHeight / 2 + frameThickness / 2, doorZ);
        fridgeGroup.add(botF);

        // Obramowanie szyby: pionowe (boki)
        const vFrameGeom = new THREE.BoxGeometry(frameThickness, doorHeight, 0.025);
        const leftF = new THREE.Mesh(vFrameGeom, blackMetalMat);
        leftF.position.set(-doorWidth / 2 + frameThickness / 2, doorY, doorZ);
        fridgeGroup.add(leftF);

        const rightF = new THREE.Mesh(vFrameGeom, blackMetalMat);
        rightF.position.set(doorWidth / 2 - frameThickness / 2, doorY, doorZ);
        fridgeGroup.add(rightF);

        // Uchwyt/Klamka pionowa (po prawej stronie)
        const handleGeom = new THREE.CylinderGeometry(0.01, 0.01, 0.45, 12);
        const handle = new THREE.Mesh(handleGeom, chromeMat);
        handle.position.set(doorWidth / 2 - 0.05, doorY, doorZ + 0.03);
        fridgeGroup.add(handle);
